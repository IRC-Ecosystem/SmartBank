...
export const connectorSpec = {
  ...
  tags: [{ name: "Health" }, { name: "User Linkage" }, { name: "Payments" }],
  components: {
    securitySchemes: {
  ...
  paths: {
    ...
    "/v1/connect/users/otp/request": { post: op("User Linkage", "Kirim OTP linkage ke inbox SmartBank", { requestBody: jsonBody(ref("OtpRequest")) }) },
    "/v1/connect/users/otp/verify": { post: op("User Linkage", "Verifikasi OTP linkage", { requestBody: jsonBody(ref("OtpVerify")) }) },
    "/v1/connect/users/link": { post: op("User Linkage", "Hubungkan user eksternal ke SmartBank Wallet", { requestBody: jsonBody(ref("LinkUser")) }) },
    "/v1/connect/users/{external_user_id}": { get: op("User Linkage", "Ambil linkage user eksternal", { parameters: [{ name: "external_user_id", in: "path", required: true, schema: { type: "string" } }] }) },
